...
public:
    ...
    MCL(void) : Node("mcl"),
        ...
        rad2deg_(180.0 / M_PI)
    {
        RCLCPP_INFO(this->get_logger(), "SM is initializing...");
        ...
        oss << "  random_particles_noise: [";
        for (size_t i = 0; i < randomParticlesNoise_.size(); ++i) {
        oss << randomParticlesNoise_[i];
        if (i != randomParticlesNoise_.size() - 1) {
            oss << ", ";
        }
        }
        oss << "]";
        ...
        for (size_t i = 0; i < odomNoiseODM_.size(); ++i) {
            oss << odomNoiseODM_[i];
            if (i != odomNoiseODM_.size() - 1) oss << ", ";
        }
        oss << "]";
        RCLCPP_INFO(this->get_logger(), "%s", oss.str().c_str());
        RCLCPP_INFO(this->get_logger(), "  use_omni_directional_model: %s", useOmniDirectionalModel_ ? "true" : "false");
        RCLCPP_INFO(this->get_logger(), "Measurement model parameters:");
        ...
        RCLCPP_INFO(this->get_logger(), "  unknown_class_prior: %.3f", pUnknownPrior_);
        ...
        RCLCPP_INFO(this->get_logger(), "  resample_thresholds: [%.3f, %.3f, %.3f, %.3f, %.3f]",
            resampleThresholds_[0], resampleThresholds_[1], resampleThresholds_[2], resampleThresholds_[3], resampleThresholds_[4]);
        RCLCPP_INFO(this->get_logger(), "  estimate_reliability: %s", estimateReliability_ ? "true" : "false");
        RCLCPP_INFO(this->get_logger(), "  rel_trans_ddm: [%.3f, %.3f]",
            relTransDDM_.size() > 0 ? relTransDDM_[0] : 0.0,
            relTransDDM_.size() > 1 ? relTransDDM_[1] : 0.0);
        RCLCPP_INFO(this->get_logger(), "  rel_trans_odm: [%.3f, %.3f, %.3f]",
            relTransODM_.size() > 0 ? relTransODM_[0] : 0.0,
            relTransODM_.size() > 1 ? relTransODM_[1] : 0.0,
            relTransODM_.size() > 2 ? relTransODM_[2] : 0.0);
